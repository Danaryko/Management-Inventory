    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Riwayat Stock In</h1>
                    <p class="mt-1 text-sm text-gray-600">
                        Riwayat aktivitas stock in yang telah Anda lakukan
                    </p>
                </div>
            </div>
        </div>
        ...
        <!-- Filter Form -->
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <form method="GET" action="{{ route('history.stock-in') }}" class="space-y-4 md:space-y-0 md:flex md:items-end md:space-x-4">
                <div class="flex-1">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Riwayat Aktivitas Stock In</h3>
                <p class="mt-1 text-sm text-gray-600">
                    Menampilkan riwayat stock in yang telah Anda lakukan
                </p>
            </div>
                            ...
                            @foreach($stockIns as $index => $stockIn)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ ($stockIns->currentPage() - 1) * $stockIns->perPage() + $index + 1 }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <div>{{ $stockIn->date->format('d/m/Y') }}</div>
                                        <div class="text-xs text-gray-500">{{ $stockIn->created_at->format('H:i:s') }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ number_format($stockIn->items->sum('quantity')) }} items
                                    </td>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Anda belum melakukan transaksi stock in atau tidak ada data pada periode yang dipilih.</p>
                </div>
            @endif
        </div>
    </div>
